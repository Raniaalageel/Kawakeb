...
//                               print("Mars",self.Mars!)
         ...
         //   self.rocketimage.image = UIImage(named: self.currentRockIMG!)
            self.Stars4.text = String (self.pointsall!)
        }


        
        n4.text = Global.shared.usercharacterName;
    ...
    func playAlarm() {
            ...
            let url = Bundle.main.url(forResource: "i4VC", withExtension: "mp3")
            // now use declared path 'url' to initialize the player
            Global.shared.audioi4VC = AVPlayer.init(url: url!)
            // after initialization play audio its just like click on play button
          //  Global.shared.audio.numberOfLoops = -1
        Global.shared.audioi4VC .play()
    }
}
